...
long Java_com_oculus_oculus360photossdk_MainActivity_nativeSetAppInterface(
		JNIEnv *jni, jclass clazz, jobject activity  )
{
	// This is called by the java UI thread.
	LOG( "nativeSetAppInterface");
	return (new Oculus360Photos())->SetActivity( jni, clazz, activity );
}
...
void Oculus360Photos::OneTimeInit( const char * launchIntent )
{
...
Matrix4f Oculus360Photos::DrawEyeView( const int eye, const float fovDegrees )
{
	...
	if ( GetUseOverlay() && CurrentPanoIsCubeMap )
	{
		...
		app->GetSwapParms().SwapOptions = ( UseSrgb ? 0 : SWAP_INHIBIT_SRGB_FRAMEBUFFER );
...
void Oculus360Photos::OnPanoActivated( OvrMetaDatum * panoData )
{
	ActivePano = panoData;
	Browser->ReloadFavoritesBuffer();
...
{
	// Save MetaData - 
	TagAction result = MetaData->ToggleTag( ActivePano, "Favorites" );
